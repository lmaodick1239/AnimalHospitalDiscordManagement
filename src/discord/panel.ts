...
export function buildPanelComponents(instanceId?: string): ActionRowBuilder<ButtonBuilder>[] {
  const id = (kind: "mode" | "rm", room?: Room) => instanceId ? panelCustomId(kind, instanceId, room) : `${kind}:${room ?? ""}`;
  ...
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(button("Mode", id("mode"))),
