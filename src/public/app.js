(() => {
  ...
  const shiftUrl = root.dataset.shift || `${location.pathname}/shift`;
  ...
  const apply = (data) => { if (data.occupancy) occupancy(data.occupancy); if (data.rows) data.rows.forEach((row) => { if (row.type === "report") { append(row.report); if (row.report.tickedAt) tick(row.report); } }); if (data.type === "report") append(data.report); if (data.type === "tick") tick(data.report); if (data.type === "shift") { const heading = root.querySelector("h1"); if (heading) heading.textContent = `SHIFT ${data.shiftNumber}`; append({ ...data.report, type: "banner", shiftNumber: data.shiftNumber }); occupancy(Object.fromEntries([...document.querySelectorAll("button[data-room]")].map((button) => [button.dataset.room, null]))); } if (data.type === "closed" || data.closed) { root.dataset.closed = "1"; document.querySelectorAll("#pad button").forEach((button) => { button.disabled = true; }); const heading = root.querySelector("h1"); if (heading && !heading.textContent.includes("Closed")) heading.textContent += " · Closed"; } };
  ...
  if (!closed) { document.querySelectorAll("button[data-room]").forEach((button) => button.addEventListener("click", async () => { button.disabled = true; try { const response = await fetch(postUrl, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ csrf, room: button.dataset.room, kind: mode }) }); const data = await response.json(); if (!response.ok || !data.ok) throw new Error(data.error || "Report failed."); apply(data); } catch (caught) { if (error) error.textContent = caught instanceof Error ? caught.message : String(caught); } finally { if (root.dataset.closed !== "1") button.disabled = false; } })); document.querySelector("#next-shift")?.addEventListener("click", async (event) => { const button = event.currentTarget; button.disabled = true; try { const response = await fetch(shiftUrl, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ csrf }) }); const data = await response.json(); if (!response.ok || !data.ok) throw new Error(data.error || "Shift advance failed."); } catch (caught) { if (error) error.textContent = caught instanceof Error ? caught.message : String(caught); } finally { if (root.dataset.closed !== "1") button.disabled = false; } }); }
  connect();
})();
