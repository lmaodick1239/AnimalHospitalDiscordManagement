...
export function instancePage(opts: { csrf: string; guildId: string; instanceId: string; shiftNumber: number; closed: boolean; occupancy: OccupancyMap; rows: SnapshotRow[]; timeZone: string; tickMark: "✅" }): string {
  ...
  const body = `<main data-tz="${esc(opts.timeZone)}" data-events="/g/${esc(opts.guildId)}/i/${esc(opts.instanceId)}/events" data-post="/g/${esc(opts.guildId)}/i/${esc(opts.instanceId)}/report" data-csrf="${esc(opts.csrf)}" data-closed="${opts.closed ? "1" : "0"}"><h1>SHIFT ${opts.shiftNumber}${opts.closed ? " · Closed" : ""}</h1>${opts.closed ? "<p class=\"closed\">Session closed.</p>" : ""}<section id="pad"><button id="mode" type="button">Mode: ANOMALY</button>${buttons}</section><p id="err" role="alert"></p><ol id="log">${rows}</ol></main><script src="/static/app.js" defer></script>`;
